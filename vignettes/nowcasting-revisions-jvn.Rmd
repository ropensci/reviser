...
requiring you to index into it. `coef()` returns the AR coefficients, the
latent-process innovation scale \(\sigma_e\), the news and noise innovation
scales, and, when selected, the diagonal spillover persistence parameters;
`vcov()` returns their covariance matrix, and `summary()` shows estimates and
...
The `states()` accessor returns the estimated state paths, optionally restricted
to named states. The state named `true_lag_0` is the current latent true value.
