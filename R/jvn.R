...
jvn_nowcast <- function(
  ...
  infer_forecast_times <- function(time_in, h) {
    ...
    if (inherits(time_in, "Date")) {
      ...
      if (length(step_m) != 1L) {
        rlang::abort(
          "Time index appears irregular; cannot infer monthly step for forecasts.",
          call = rlang::caller_env()
        )
      }
      seq.Date(
  ...
  transform_params_and_cov <- function(params_raw, cov_raw, model_struct, transform_se) {
    params <- params_raw
    cov_used <- cov_raw
    ...
    if (!is.null(cov_used)) {
      ...
      # Diagonal-only fallback if we had raw SEs but no cov: caller can overwrite.
      se <- rep(NA_real_, length(params_raw))
    }
  ...
  if (!is.null(solver_options$score_method) && is.null(solver_options$qml_score_method)) {
  ...
  if (length(bad) > 0) {
    rlang::abort(
      paste0(
        ...
        "Valid options are: ", paste(names(default_solver_options), collapse = ", "), "."
      ),
      call = rlang::caller_env()
    )
  }
  ...
  abort_scalar(default_solver_options$trace, "solver_options$trace", \(x) x >= 0,
               "must be a single number >= 0.")
  default_solver_options$trace <- as.integer(round(default_solver_options$trace))
  
  abort_scalar(default_solver_options$maxiter, "solver_options$maxiter", \(x) x >= 1,
               "must be a single integer >= 1.")
  default_solver_options$maxiter <- as.integer(round(default_solver_options$maxiter))
  
  abort_scalar(default_solver_options$n_starts, "solver_options$n_starts", \(x) x >= 1,
               "must be a single integer >= 1.")
  default_solver_options$n_starts <- as.integer(round(default_solver_options$n_starts))
  
  abort_scalar(default_solver_options$qml_eps, "solver_options$qml_eps", \(x) x > 0,
               "must be > 0.")
  
  abort_scalar(default_solver_options$sigma_lower, "solver_options$sigma_lower", \(x) x > 0,
               "must be > 0.")
  abort_scalar(default_solver_options$sigma_upper, "solver_options$sigma_upper",
  ...
  if (is.matrix(df)) {
    df <- data.frame(time = seq_len(nrow(df)), 
                     as.data.frame(df), 
                     check.names = FALSE)
    ...
    for (start_idx in seq_len(n_starts)) {
      ...
      if (opt_method == "two-step") {
        opt_nm <- stats::optim(
          ...
          transform_se = default_solver_options$transform_se,
          method = "Nelder-Mead",
          control = list(trace = max(0, default_solver_options$trace), maxit = 500)
        )
        ...
        current_result <- stats::optim(
          ...
          transform_se = default_solver_options$transform_se,
          method = "BFGS",
          control = list(trace = max(0, default_solver_options$trace), 
  ...
  if (se_method_used == "hessian") {
    h_res <- tryCatch(
      compute_hessian_cov(params_raw, 
                          model_struct, 
                          y_mat, 
                          default_solver_options$transform_se),
      error = function(e) list(cov = NULL, 
                               warning = paste0("Hessian computation failed: ", 
                                                e$message))
    )
    cov_raw <- h_res$cov
  ...
  # If cov is missing but we do have raw SEs, apply diagonal delta fallback for SDs.
  se <- tr$se
  ...
  R_kfas <- array(model_struct$R, dim = c(model_struct$m, ncol(model_struct$R), 1))
  Q_kfas <- array(model_struct$Q, dim = c(ncol(model_struct$R), ncol(model_struct$R), 1))
...
jvn_matrices <- function(
  ...
  # ===== Parameter indexing =====
  idx <- 1L
  ar_coef_idx <- idx:(idx + ar_order - 1L); idx <- idx + ar_order
  sigma_e_idx <- idx; idx <- idx + 1L
  
  sigma_nu_idx <- NULL
  if (isTRUE(include_news)) {
    sigma_nu_idx <- idx:(idx + n_vint - 1L); idx <- idx + n_vint
  }
  
  sigma_zeta_idx <- NULL
  if (isTRUE(include_noise)) {
    sigma_zeta_idx <- idx:(idx + n_vint - 1L); idx <- idx + n_vint
  }
  
  spill_nu_idx <- NULL
  if (isTRUE(include_spillovers) && isTRUE(include_news) && isTRUE(spillover_news)) {
    spill_nu_idx <- idx:(idx + n_vint - 1L); idx <- idx + n_vint
  }
  
  spill_zeta_idx <- NULL
  if (isTRUE(include_spillovers) && isTRUE(include_noise) && isTRUE(spillover_noise)) {
    spill_zeta_idx <- idx:(idx + n_vint - 1L); idx <- idx + n_vint
  }
  ...
  news_names <- if (isTRUE(include_news))  paste0("news_vint",  1:n_vint) else NULL
  noise_names <- if (isTRUE(include_noise)) paste0("noise_vint", 1:n_vint) else NULL
...
jvn_update_matrices <- function(model_struct, params) {
  ...
  if (isTRUE(info$include_noise)) {
    ...
    R[noise_start:(noise_start + info$n_vint - 1L),
      shock_start:(shock_start + info$n_vint - 1L)] <- 0
...
#' @keywords internal
#' @noRd
jvn_kalman_loglik <- function(y, Z, Tmat, R, Q, H = NULL, a1 = NULL, P1 = NULL) {
  y <- as.matrix(y)
  n <- nrow(y)
  p <- ncol(y)
  m <- nrow(Tmat)
  ...
  for (t in seq_len(n)) {
    ...
    cholF <- tryCatch(chol(F), error = function(e) NULL)
    if (is.null(cholF)) {
      base <- 1e-10 * max(1, mean(abs(diag(F))))
      for (j in 0:4) {
        cholF <- tryCatch(chol(F + (10^j) * base * diag(k)), 
...
#' @keywords internal
#' @noRd
jvn_negloglik_contrib <- function(params, 
                                  model_struct, 
                                  data, 
                                  transform_se = TRUE) {
  ...
  if (isTRUE(transform_se)) {
    idx_sd <- unlist(c(info$sigma_e_idx, info$sigma_nu_idx, info$sigma_zeta_idx))
    idx_sd <- idx_sd[is.finite(idx_sd)]
  ...
  s <- sum(rho)
  if (!is.finite(s) || abs(s) >= 0.999) return(rep(1e8, Tn))
  ...
  if (is.null(P0)) return(rep(1e8, Tn))
  
  # Filter to get per-time loglik contributions, then flip sign to objective contributions
...
jvn_standardize <- function(Y) {
  ...
  if (!is.finite(scale) || scale <= 0) {
    rlang::abort(
      "Standard deviation of the final vintage is not positive; cannot standardize."
    )
  }
...
jvn_init_params <- function(model_struct, data, transform_se = TRUE) {
  ...
  fallback <- function() {
    ...
    if (isTRUE(transform_se)) {
      idx_sd <- unlist(c(info$sigma_e_idx, 
                         info$sigma_nu_idx, 
                         info$sigma_zeta_idx))
      idx_sd <- idx_sd[is.finite(idx_sd)]
  ...
  if (nrow(data_clean) < ar_order + 10) {
    warning("Insufficient data for smart initialization. Using fallback values.")
    return(fallback())
  }
  ...
  if (is.null(ar_fit) || length(ar_fit$ar) < ar_order || any(!is.finite(ar_fit$ar))) {
  ...
  # GAUSS-like constraint spirit used in your likelihood: keep sum(rho) in (-1,1)
  ...
  if (isTRUE(info$include_spillovers)) {
    ...
    if (isTRUE(info$include_news) && 
        isTRUE(info$spillover_news) && !is.null(info$spill_nu_idx)) {
    ...
    if (isTRUE(info$include_noise) && 
        isTRUE(info$spillover_noise) && !is.null(info$spill_zeta_idx)) {
...
#' the score contributions are rescaled accordingly before the sandwich matrix is
#' formed.
#'
#' @keywords internal
#' @noRd
jvn_qml_covariance <- function(theta_hat,
                               model_struct,
                               y,
                               transform_se = TRUE,
                               score_eps = 1e-4,
  ...
  base_contrib <- jvn_negloglik_contrib(theta_hat, model_struct, y, transform_se)
  ...
  for (j in seq_len(k)) {
    ...
    c_plus <- jvn_negloglik_contrib(theta_hat + step, model_struct, y, transform_se)
    
    if (length(c_plus) != nT) {
      stop("jvn_negloglik_contrib returned different length under perturbation; 
           cannot form scores.")
    }
    
    if (score_method == "central") {
      c_minus <- jvn_negloglik_contrib(theta_hat - step, model_struct, y, transform_se)
...
jvn_param_table <- function(params, se, param_info) {
  ...
  if (isTRUE(param_info$include_news)) {
    param_names <- c(param_names, paste0("sigma_nu_", seq_len(param_info$n_vint)))
  }
  
  # Noise shocks
  if (isTRUE(param_info$include_noise)) {
    param_names <- c(param_names, paste0("sigma_zeta_", seq_len(param_info$n_vint)))
  }
